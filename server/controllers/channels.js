...
export const send_media_files = async (req, res) => {
	...
	for(let i = 0 ; i < req.files.length ; i++) {
		...
		fs.renameSync(path.resolve(process.cwd(), req.files[i].path), path.resolve(process.cwd(), `..\\client\\public\\data\\channels\\${channel_media_id}.${extension}`))
...
export const delete_channel = async (req, res) => {
	let rows1 = await channelsModel.media.select(req.body.conv_id, 10000000000, true)
	rows1.forEach(file => {
	...
	let status1 = await channelsModel.media.delete_all(req.body.conv_id)
	let status2 = await channelsModel.members.delete_all_by_founder(req.body.conv_id)
	let status3 = await channelsModel.delete_channel(req.body.conv_id)
