...
groupsController.send_media_message = async (req, res) => {
	let group_media_id = await groupsModel.media.insert(req.body.conv_id, req.cookies.user_id, 1, req.body.text) //encrypted_message
...
groupsController.send_media_files = async (req, res) => {
	...
	for(let i = 0 ; i < req.files.length ; i++) {
		...
		let group_media_id = await groupsModel.media.insert(req.body.conv_id, req.cookies.user_id, media_type_id, extension)

		console.log(process.cwd(), req.files[i].path, path.resolve(process.cwd(), req.files[i].path))
		fs.renameSync(path.resolve(process.cwd(), req.files[i].path), path.resolve(process.cwd(), `..\\client\\public\\data\\groups\\${group_media_id}.${extension}`))
...
groupsController.add_member = async (req, res) => {   //add_member //join
	...
	let status = 'failure'
	if(user) {
		status = await groupsModel.members.insert(req.body.group_id, user.user_id)
	}
	res.send({status: status, user: user})
}
groupsController.leave = async (req, res) => {
	let status1 = await groupsModel.members.delete(req.body.conv_id, req.cookies.user_id)
	let rows2 = await groupsModel.members.select(req.body.conv_id)
	if (rows2.length == 0) {
		// also delete all media files in data folder
		let rows3 = await groupsModel.media.select(req.body.conv_id, 10000000000, true)
		rows3.forEach(file => {
			if(file.media_type != 'message') {
				fs.unlinkSync(path.resolve(process.cwd(), `..\\client\\public\\data\\groups\\${file.media_id}.${file.text}`))
			}
		})
		let status2 = await groupsModel.media.delete(req.body.conv_id)
		let status3 = await groupsModel.delete(req.body.conv_id)
	}
