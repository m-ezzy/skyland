...
export async function select_new(user_id, q) {
	...
	let query = `SELECT channels.channel_id,channels.channel_name,channels.title,channels.extension FROM channels INNER JOIN channel_members ON channels.channel_id=channel_members.channel_id WHERE channel_members.user_id != ${user_id} AND (channels.channel_name LIKE '%${q}%' OR channels.title LIKE '%${q}%')`
	let rows = await execute_query(query)
	return rows
}/*
