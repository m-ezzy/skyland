...
export default async function() { //search_previous + search_new //handle_click_search
	...
	data.forEach(d => {
		...
		let hero = create_hero(this.names, d.conv_id, d.conv_name, d.title, d.extension, '', '', true)

		if(this.names != 'groups') {
			let classes = 'create'
			let click_handler = `chats.handle_click_create_chats(${d.conv_id}, '${d.conv_name}', '${d.title}', '${d.first_name}', '${d.last_name}', '${d.extension}')`
			let text = 'create'
	...
	if(!match_found || (this.names == 'chats' && !match_found && data.length == 0)) {
		let num = this.names == 'chats' ? 5 : 4
		sections[num].classList.remove('hidden')
	}
}
