...
export default async function() { //message = this.tm.value, key = this.tk.value, chat_id = this.current
	...
	let data = await fetch_data(`/${this.names}/send_media/message`, {conv_id: conv_id, media_type_id: 1, text: text}) //encrypted_message //text
	this.e.t.msg.value = ''
	...
	this.add_media(value) // this.item_media(value, this.names != 'chats')
	socket[this.names].emit('send-media', value)
	this.e.sound.cf.play()
}
