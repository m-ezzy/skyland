...
callsModel.groups.select = async (user_id) => {
	...
	let query = `SELECT calls_groups.group_id AS conv_id,calls_groups.user_id,calls_groups.call_type,groups.group_name AS conv_name,groups.title,groups.extension FROM calls_groups INNER JOIN groups ON calls_groups.group_id=groups.group_id WHERE groups.group_id in (SELECT group_id FROM group_members WHERE user_id=${user_id}) ORDER BY time_created DESC`
	let rows = await execute_query(query)
	return rows
}
//insert
...
//update
callsModel.chats.update = async (chat_id, user_id, call_type, time) => {
