...
chatsModel.select_new = async (user_id, q) => { //select_search_new //this should be in users model or not
	let query = `SELECT user_id AS conv_id,user_name AS conv_name,first_name,last_name,extension FROM users WHERE user_name LIKE '%${q}%' OR first_name LIKE '%${q}%' OR last_name LIKE '%${q}%'`
...
chatsModel.delete_all = async (user_id) => {
	let query = `DELETE FROM chats WHERE user_id1=${user_id} OR user_id2=${user_id}`
	let rows = await execute_query(query)
	return "success"
}
...
chatsModel.media.select = async (chat_id, row_up, select_all) => {
