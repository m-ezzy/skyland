...
chatsController.delete = async (req, res) => {
	let status1 = ''
	if (req.body.block) {
		status1 = await chatsModel.update(req.body.conv_id, req.cookies.user_id)
	} else {
		let status2 = await callsModel.chats.delete(req.body.conv_id)
		...
		let status3 = await chatsModel.media.delete(req.body.conv_id)
		let status4 = await chatsModel.delete(req.body.conv_id)
	}
